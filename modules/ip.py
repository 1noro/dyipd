...
def get_my_ip_from_gmail(verbose):
    ...
    port = 25
    if verbose: print("[INFO] getting my ip from "+host+":"+str(port))
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect((host, port))
    rdata = sock.recv(1024)
    if verbose: print('[ <--] ', str(rdata))
...
def check_ip(file, verbose):
    ...
    if os.path.isfile(file):
        ...
        if verbose: print("[INFO] last ip: "+mylastip)
        ip_from_gmail = get_my_ip_from_gmail(verbose)
        if verbose: print("[INFO] ip from gmail: "+ip_from_gmail)
        if mylastip != ip_from_gmail:
            myip_change = True
            myip = ip_from_gmail
            if verbose: print("[INFO] the ip has changed to: "+myip)
            f = open(file, "w+")
            f.write(myip+"\n")
            f.close()
            if verbose: print("[INFO] '"+file+"' saved with the new ip")
        else:
            myip = mylastip
            if verbose: print("[INFO] the ip has not changed")
    else:
        myip_change = True
        if verbose: print("[WARN] '"+file+"' not found, creating one...")
        ip_from_gmail = get_my_ip_from_gmail(verbose)
        if verbose: print("[INFO] ip from gmail: "+ip_from_gmail)
        myip = ip_from_gmail
        if verbose: print("[INFO] the new ip is: "+myip)
        f = open(file, "w+")
        f.write(myip+"\n")
        f.close()
        if verbose: print("[INFO] '"+file+"' saved with the new ip")
